...
//Message de sortie simple 
	$logout_message = "<div class='msgln'><span class='left-info'>User <b class='user-name-left'>" . $_SESSION['ep'] . "</b> a quitté la session de profil.</span><br></div>"; 
...
function loginForm() { 
	echo '<div id="loginform"> <p>Veuillez saisir votre nom pour continuer!</p> <form action="profil.php" method="post"> <label for="name">Nom: </label> <input type="text" name="name" id="name" /> <input type="submit" name="enter" id="enter" value="Soumettre" /> </form> </div>'; 
} 
?> 
...
<head> 
	<meta charset="utf-8" /> 
	<title>Profil de votre ami</title> 
...
<body> 
	...
	include('monreseau.php');

$ami_id = $_GET['id'];

$sql = "SELECT * FROM auteur WHERE email_auteur = $emai_auteur";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    $ami = $result->fetch_assoc();
} else {
    echo "Aucun ami trouvé avec cet identifiant.";
    exit();
}
...
?> 
		<div id="wrapper"> 
			...
			<div id="menu">
				...
				<p class="logout"><a id="exit" href="#">Quitter la page</a></p> 
